...
void SPPM::prePass(int samples, int offset, bool adaptive)
{
	...
	if (bHashgrid) {
		Y_INFO << integratorName << ": Building photon hashgrid..." << yendl;
	}
	else {
		Y_INFO << integratorName << ": Building photon map..." << yendl;
	}
...
integrator_t* SPPM::factory(paraMap_t &params, renderEnvironment_t &render)
{
	...
	//
	params.getParam("bHashgrid", hashGrid);
