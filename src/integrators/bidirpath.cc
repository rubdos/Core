...
colorA_t biDirIntegrator_t::integrate(renderState_t &state, diffRay_t &ray) const
{
    ...
    if(scene->intersect(testray, sp))
    {
        ...
        // create eyePath
        nEye = createPath(state, ray, pathData.eyePath, MAX_PATH_LENGTH); // MAX_PATH_LENGTH
		...
		if (dbg < 10) {
			std::cout << "lightNumPdf=" << lightNumPdf << std::endl;
		}
        ++dbg;
        ...
        if (do_lightImage)
        {
            ...
            for(int s=2; s<=nLight; ++s)
            {
				...
				if (!connectPathE(state, s, pathData)) {
					continue;
				}
...
int biDirIntegrator_t::createPath(renderState_t &state, ray_t &start, std::vector<pathVertex_t> &path, int maxPathLen) const
{
    ...
    while(nVert < maxPathLen)
    {
        ...
        if(nVert > MIN_PATH_LENGTH)
        {
			...
			if (prng() > v.qi_wo) {
				break; // terminate path with russian roulette
			}
        }
		else {
			v.qi_wo = 1.f;
...
inline bool biDirIntegrator_t::connectPathE(renderState_t &state, int s, pathData_t &pd) const
{
    ...
    ray_t wo(z.sp.P, -vec);
	if (!cam->project(wo, 0, 0, pd.u, pd.v, x_e.pdf_b)) {
		return false;
	}
